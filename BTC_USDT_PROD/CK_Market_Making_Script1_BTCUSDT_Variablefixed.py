...
Bulk_Buy_Order(10,lower_bound_amt,upper_bound_amt,LTP-price_regulator2,LTP-price_regulator,market_name)
print("\n \n \t BULK BUY ORDERS HAS BEEN PLACED \n \n")
Bulk_Sell_Order(10,lower_bound_amt,upper_bound_amt,LTP+price_regulator,LTP + price_regulator2,market_name)
    ...
    while(True):
        try:
            ...
            if(Order_len >10):
                ...
                Bulk_Buy_Order(2,lower_bound_amt,upper_bound_amt,LTP-price_regulator2,LTP-price_regulator,market_name)
                time_quantum_delay()
                
                order_id = Pending_Orders_User(client_id,client_secret,market_name,user_id)[0]
                Cancel_Order(client_id,client_secret,market_name,order_id)
                Bulk_Sell_Order(2,lower_bound_amt,upper_bound_amt,LTP+price_regulator,LTP + price_regulator2,market_name)              
                time_quantum_delay()
            else:
                print(Order_len)
                Bulk_Buy_Order(4,lower_bound_amt,upper_bound_amt,LTP-price_regulator2,LTP-price_regulator,market_name)
                Bulk_Sell_Order(4,lower_bound_amt,upper_bound_amt,LTP+price_regulator,LTP + price_regulator2,market_name)               
